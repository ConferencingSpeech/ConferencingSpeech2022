...
def check_audio_order(csv1,csv2):
    ...
    for i in range(len(df1)):
        if df1.iloc[i]['deg_wav'] !=df2.iloc[i]['deg_wav']:
            print(df1.iloc[i]['deg_wav'])
            print(df2.iloc[i]['deg_wav'])
            return False
    return True
if __name__=='__main__':
    evaluate_mos_dir('D:\\voiceQuality\\无参考语音质量评估竞赛\\提交结果','D:\\voiceQuality\\无参考语音质量评估竞赛\\ConferencingSpeech2022\\eval_result_mos_msft_tub_tencent.csv')
